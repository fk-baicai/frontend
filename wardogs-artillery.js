(function () {
    ...
    function toggleLock(kind) {
        if (kind === 'tgt') state.lockTgt = !state.lockTgt;
        else state.lockGun = !state.lockGun;
        syncLockUi();
        save();
        draw();
    }
    ...
    function draw() {
        ...
        var map = currentMap();
        ctx.filter = 'brightness(1.18) contrast(1.14)';
        ...
        function drawMission(m, index, faded) {
            ...
            if (m.gun) {
                ...
                ctx.fillStyle = 'rgba(247, 143, 30, 0.02)';
                ctx.fill('evenodd');
                ctx.setLineDash([6, 5]);
    ...
    function setPlace(id) {
        ...
        document.querySelectorAll('[data-wd-place]').forEach(function (btn) {
            btn.classList.toggle('is-active', btn.getAttribute('data-wd-place') === id);
        });
        save();
    }
    ...
    function clearPoints() {
        if (!state.lockGun) state.gun = null;
        if (!state.lockTgt) state.tgt = null;
        writeInputs();
        update();
    }
